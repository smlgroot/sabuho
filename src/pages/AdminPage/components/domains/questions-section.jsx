    ...
    <div className="card bg-base-100 border">
      <div className="card-body">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    <span className="hidden sm:inline">{t('Advanced Filters')}</span>
              ...
              <div className="space-y-4 pt-3 border-t border-border/40">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  <div className="space-y-2 lg:col-span-2">
                    <label className="text-sm font-medium text-muted-foreground">{t('Created Date Range')}</label>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                      <div className="flex-1">
                        <label htmlFor="date-from" className="sr-only">{t('From Date')}</label>
                        <input
                          id="date-from"
                          type="date"
                      ...
                      <div className="flex-1">
                        <label htmlFor="date-to" className="sr-only">{t('To Date')}</label>
                        <input
                          id="date-to"
                          type="date"
                          className="input input-bordered text-sm w-full"
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-muted-foreground">{t('Question Type')}</label>
                    <div className="dropdown w-full">
                      ...
                      >
                        <span className="truncate">
                          {resourceIdFilter === 'all' ? t('All Questions') : 
                           resourceIdFilter === 'manual' ? t('Manual') : t('Auto Generated')}
                        </span>
        ...
        {filteredAndSortedQuestions.length > 0 ? (
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 p-2 bg-gray-50 rounded-md">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">
                  {selectedQuestions.size > 0
                    ? t('{{selected}} of {{total}} selected', { selected: selectedQuestions.size, total: filteredAndSortedQuestions.length })
                    : t('Select All')}
                </span>
              </div>
              {selectedQuestions.size > 0 && (
          ...
          <p className="text-muted-foreground text-sm">{t('No Questions Match Your Search')}</p>
        ) : (
          <p className="text-muted-foreground text-sm">{t('No Questions Created Yet')}</p>
        )}
      </div>
    </div>
  )
